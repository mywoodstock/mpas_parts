...
def read_input_row_col_val(filename, data):
	...
	while True:
		...
		count += 1
		words = line.split()
		data += [(int(words[0]), int(words[1]), float(words[2]))]	# assuming vals == 1 only
	if count != nnz:
...
def matrix_add(data1, data2, result, size):
	mat1 = sets.Set(data1)
	mat2 = sets.Set(data2)
	mat3 = mat1.union(mat2)
	result += list(mat3)
...
result = []
matrix_add(data1, data2, result, size1)
